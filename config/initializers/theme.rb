...
require_if_exists Configuration.theme_path.join('simple_form.rb')

# Add theme assets to the beginning of the sprockets load path
Rails.configuration.assets.paths =
  Dir[Configuration.theme_path.join("assets/**/*")] +
  Rails.configuration.assets.paths

# Add theme i18n
Rails.configuration.i18n.load_path +=
  Dir[Configuration.theme_path.join('locales/**/*')]

# Load theme's submission adapters
Dir[Configuration.theme_path.join('submission_adapters/**/*')].each do |f|
  require f
end
